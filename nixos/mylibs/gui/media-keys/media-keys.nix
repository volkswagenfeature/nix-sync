...
let
  ...
  brightness = stdenv.mkDerivation rec {
      pname = "brightness";
      ...
      configurePhase= ''
        mkdir -p $out/bin
        cp -t $out/bin/ brightness_arr.sh 
        chmod +x $out/bin/brightness_arr.sh
        SPECIAL=$( ./generator.sh )
        substituteInPlace $out/bin/brightness_arr.sh \
          --replace "wobfifo"  "$wobfifo"\
...
in
{
  enviroment.systemPackages = [pkgs.wob brightness vol-control];
  ...
  home-manager.users."${secrets.primaryuser}"= {pkgs, ...}:{
    wayland.windowManager.sway.config = {
      ...
      keybindings = lib.mkOptionDefault {
        "XF86MonBrightnessUp"   = "exec ${brightness}/bin/brightness 1";
        "XF86MonBrightnessDown" = "exec ${brightness}/bin/brightness -1";
